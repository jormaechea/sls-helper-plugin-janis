...
module.exports = {
  ...
  #set( $filtersLog = "" )
  #foreach( $qk in $qs.keySet() )
    #if( $qk.startsWith("filters") )
      #if( $filtersLog != "" )#set( $filtersLog = "$filtersLog&" )#end
      #set( $filtersLog = "$filtersLog$qk=$qs.get($qk)" )
    #end
  #end
  #if( $filtersLog.length() > 256 )#set( $filtersLog = $filtersLog.substring(0, 256) )#end
  #if( $filtersLog != "" )#set( $context.requestOverride.querystring.filters = $util.escapeJavaScript($filtersLog).replaceAll("\\\\'", "'") )#end
  #set( $body = $input.json("$") )
  ${rawBody}
