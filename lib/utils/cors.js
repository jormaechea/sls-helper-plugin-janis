...
const makeUserDefinedCors = ({ replace, ...userDefinedCors }) => {
	...
	if(!replace)
		return { corsConfiguration: userDefinedCors };
	...
	return {
		...
		maxAge: userDefinedMaxAge === false ? null : (userDefinedMaxAge || defaultMaxAge)
	};
};
...
module.exports = userDefinedCors => {
	...
	if(typeof userDefinedCors === 'object' && !Array.isArray(userDefinedCors))
