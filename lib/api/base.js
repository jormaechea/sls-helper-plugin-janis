...
const apiBase = ({
	...
	if(!entityName)
		throw new Error(`Missing or empty entityName hook configuration in ${hookName} hook`);

	if(!method)
		throw new Error(`Missing or empty method hook configuration in ${hookName} hook`);

	const entityNameAsTitle = startcase(entityName);
	const entityNameAsTitleWithoutSpaces = entityNameAsTitle.replace(/ /g, '');
	...
	const functionConfiguration = {
		name: `\${self:custom.serviceName}-\${self:custom.stage}-${getFunctionNamePrefix(methodName)}${entityNameAsTitleWithoutSpaces}`,
	...
	return {
		[`API${getFunctionNamePrefix(methodName)}-${entityNameAsTitleWithoutSpaces}`]: functionConfiguration
	};
};
