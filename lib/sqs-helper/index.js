...
module.exports = class SQSHelper {
	...
	static buildConsumerFunction({
		...
		const getMaximumBatchingWindow = ({ fastProcessingEnvironments: fpe, maximumBatchingWindow: mbw }) => {
			if(fpe?.includes(process.env.ENV))
				return 0;
		...
		return ['function', {
			...
			events: [
				this.createEventSource(queueArn, { batchSize, maximumBatchingWindow: getMaximumBatchingWindow({ fastProcessingEnvironments, maximumBatchingWindow }), eventProperties }),
				...mainConsumer && this.delayConsumerProperties?.useMainHandler ? [this.createEventSource(this.arns.delayQueue, { ...this.delayConsumerProperties, maximumBatchingWindow: getMaximumBatchingWindow(this.delayConsumerProperties) })] : [],
				...mainConsumer && this.dlqConsumerProperties?.useMainHandler ? [this.createEventSource(this.arns.dlq, { ...this.dlqConsumerProperties, maximumBatchingWindow: getMaximumBatchingWindow(this.dlqConsumerProperties) })] : []
			],
			...functionProperties
		}];
	}
