...
module.exports = class SQSHelper {
	...
	static buildHooks(configs = {}) {
		...
		if(this.shouldAddConsumer(this.dlqConsumerProperties)) {
			...
			dlqConsumerHooks.push(this.buildQueueResource(this.dlqConsumerDLQProperties, { dlqConsumer: true }));
		}
	...
	static validateConfigs(configs) {
		...
		[
			...
			['DLQ Consumer DLQ', configs.dlqConsumerDLQProperties]
	...
	static setConfigsWithDefaults(userConfigs) {
		...
		this.dlqConsumerDLQProperties = userConfigs.dlqConsumerProperties
			? { ...dlqConsumerDLQDefaultsValue, ...userConfigs.dlqConsumerDLQProperties }
			: null;
