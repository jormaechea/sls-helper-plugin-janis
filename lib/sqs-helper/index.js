...
module.exports = class SQSHelper {
	...
	static buildHooks(configs = {}) {
		...
		if(this.shouldAddConsumer(this.dlqConsumerProperties)) {
			...
			dlqConsumerHooks.push(this.buildQueueResource(this.dlqConsumerDLQQueueProperties, { dlqConsumer: true }));
		}
	...
	static validateConfigs(configs) {
		...
		[
			...
			['DLQ Consumer DLQ Queue', configs.dlqConsumerDLQQueueProperties]
	...
	static setConfigsWithDefaults(userConfigs) {
		...
		this.dlqConsumerDLQQueueProperties = { ...dlqConsumerDLQQueueDefaultsValue, ...userConfigs.dlqConsumerDLQQueueProperties };
	...
	static buildQueueResource({
		...
		if(mainQueue) {
			...
			name = this.names.dlqConsumerDlq;
			deadLetterTargetArn = this.arns.dlqConsumerDlq;
		} else {
			// dlq
			name = this.names.dlq;
		}
		...
		if(dlq)
			...
			SQSType = SQSTypes.DLQConsumerDLQ;
